...
Pioneers} (C\&P) {[}@Nugent1979\_cavaliers3{]}. Without geographic
...
\item
  ...
  {[}@Bashorun2025\_gis{]}, deterministic pipelines such as the GeoTxt
  Stanford NER geoparser {[}@karimzadeh2019geotxt{]}, neural geoparsers
  like Mordecai-3 {[}@halterman2023mordecai{]}, and other heuristic
...
{[}@Li2024\_geoai{]} and detailed further in §2.4, and---relative to
classic geoparsers such as Stanford NER/GeoTxt
{[}@karimzadeh2019geotxt{]}, Mordecai‑3 {[}@halterman2023mordecai{]},
and the map‑aware CamCoder {[}@Gritta2018\_camcoder{]}---centers on
...
\item
  ...
  \emph{Cavaliers and Pioneers}, Vol. 3 {[}@Nugent1979\_cavaliers3{]},
...
{[}@Julian2014\_tennessee{]} required nearly ten years of archival
...
utility for spatial analysis {[}@lva\_patents\_db{]}. Similarly, Loudoun
County GIS staff have successfully reconstructed all original grants
within their jurisdiction {[}@loudoun\_grants\_dataset{]}. These
...
proprietary are Mitchell's {[}@mitchell1977whiteoak{]} maps and
...
Virginia, seminal studies like Fausz {[}@Fausz1971\_settlement{]}
...
{[}@Dobbs2009\_backcountry{]} used georeferenced North Carolina grants
...
DeedMapper software {[}@DeedMapper\_software{]} helps researchers
...
names. Neural architectures such as CamCoder {[}@Gritta2018\_camcoder{]}
...
{[}@Hu2024\_toponym\_llm{]}, who fine-tuned Llama 2-7B to generate an
...
on the GeoCorpora set {[}@wallgrun2017geocorpora{]}, outperforming
...
corpus {[}@gritta2018whatsmissing{]} the same architecture reached 0.98.
Crucially, these gains were realized on commodity hardware---the entire
experiment ran on a single NVIDIA V100 with 14 GB VRAM, showing that
parameter‑efficient fine‑tuning is feasible without data‑centre
...
{[}@wu2025geosg{]} introduced GeoSG, a self-supervised graph neural
...
{[}@savarro2024geolingit{]} demonstrated that Italian tweets can be
...
al. {[}@Osullivan2024\_metric{]} demonstrated that GPT-class models
...
and WikToR {[}@wallgrun2017geocorpora; @gritta2018whatsmissing{]}. By
...
next section (§ 2.3) explores tool‑augmented prompting frameworks that
grant LLMs access to external geocoders and vector
...
{[}@Hu2024\_toponym\_llm{]}, who coupled a fine-tuned Llama 2-7B with a
cascading trio of geocoders---GeoNames {[}@geonames\_about{]}, Nominatim
{[}@nominatim\_project{]}, and ArcGIS Online
{[}@esri\_arcgis\_online{]}---to resolve toponyms the model had already
...
Huang et al. {[}@Huang2024\_geoagent{]} developed GeoAgent for free-form
...
{[}@li2023geogluegeographiclanguageunderstanding{]} and an in-house
...
pattern known as the ReAct prompting paradigm {[}@yao2023react{]}, which
...
initiative {[}@GoogleResearch2025\_geospatial{]} exemplifies the
...
(Hu et al. {[}@Hu2024\_toponym\_llm{]}; Huang et al.
{[}@Huang2024\_geoagent{]}). The present work builds on this pattern by
...
{[}@Huang2024\_geoagent;
@li2023geogluegeographiclanguageunderstanding{]}, our outputs are point
estimates aligned to archival ground truth for multi‑sentence, long‑form
descriptions.
...
responsibility}, which Li et al. {[}@Li2024\_geoai{]} identify as four
...
\emph{Cavaliers \& Pioneers}, Volume 3 {[}@Nugent1979\_cavaliers3{]}
contains 5,471 abstracts of Virginia land patents recorded in patent
books 9--14 (1695--1732). The digitized corpus
{[}@mioduski\_2025\_cvp3{]} provides machine-readable versions of these
...
History {[}@central\_va\_patents\_gis{]}. Matching relied on grantee
name, grant year, and acreage. Each candidate polygon was visually
audited against modern hydrography, historic county boundaries, and the
...
A certified GIS analyst {[}@Bashorun2025\_gis{]} implemented an
...
{[}@halterman2023mordecai{]}, augmented with domain-specific heuristics
...
\caption{\label{tbl:cost}Cost metrics by method---cost per located grant
(USD) and cost per 1,000 located grants (USD), with mean error
(km).}\tabularnewline
\toprule\noalign{}
...
\item
  ...
  Method rankings remain unchanged; only H-1 {[}@Bashorun2025\_gis{]}
...
\item
  ...
  Shared Story {[}@central\_va\_patents\_gis{]} and other spatial
  datasets used for verification could potentially appear in training
...
\item
  \textbf{Model family scope.} Our LLM experiments are restricted to
  OpenAI's GPT/o‑series snapshots from April--May 2025. This choice is
  methodological rather than proprietary: cross‑provider comparisons
  confound architecture with implementation details that are neither
  standardized nor fully documented. Tokenization schemes,
  context‑window management, nucleus/temperature scheduling,
  system‑prompt handling, tool‑call middleware, safety filters, and
  log‑prob reporting differ subtly across vendors and often across
  endpoints within the same vendor. Even when nominal hyperparameters
  share names (e.g., temperature, top‑p, frequency/penalty), their
  effective semantics---clamping ranges, annealing schedules,
  interaction with decoding heuristics---are provider‑specific. Holding
  to a single provider keeps the evaluation manifold fixed: identical
  tokenization, identical server‑side decoding policies, identical
  tool‑call semantics and error surfaces, and identical billing/latency
  instrumentation. This design decision reduces between‑system variance
  and preserves internal validity for cost/latency/accuracy trade‑offs
  on historical abstracts. Results may not transfer to other proprietary
  or open‑source families (e.g., Llama, Mistral) without replication
  under equivalently controlled conditions; we treat cross‑model
...
\item
  ...
  {[}@Bashorun2025\_gis{]} relies on a single expert-authored geocoding
...
\item
  ...
  \emph{Cavaliers and Pioneers} {[}@Nugent1979\_cavaliers3{]}---and
...
are deeply grateful to Bimbola Bashorun {[}@Bashorun2025\_gis{]} for
...
professional GIS benchmark (H-1) {[}@Bashorun2025\_gis{]} results,
...
\bibitem[\citeproctext]{ref-esri_arcgis_online}
Esri. (2025). \emph{ArcGIS online}. Cloud-based GIS platform.
...
\bibitem[\citeproctext]{ref-gritta2018whatsmissing}
Gritta, M., Pilehvar, M. T., \& Collier, N. (2018a). What's missing in
geographical parsing? \emph{Language Resources and Evaluation},
\emph{52}(2), 603--623. \url{https://doi.org/10.1007/s10579-017-9385-8}

\bibitem[\citeproctext]{ref-Gritta2018_camcoder}
Gritta, M., Pilehvar, M. T., \& Collier, N. (2018b). Which melbourne?
...
\bibitem[\citeproctext]{ref-li2023geogluegeographiclanguageunderstanding}
Li, D., Ding, R., Zhang, Q., Li, Z., Chen, B., Xie, P., Xu, Y., Li, X.,
Guo, N., Huang, F., \& He, X. (2023). \emph{GeoGLUE: A GeoGraphic
...
\bibitem[\citeproctext]{ref-nominatim_project}
OpenStreetMap Contributors. (2025). \emph{Nominatim: Open-source
geocoding with OpenStreetMap data}. Open-source geocoding software.
...
\bibitem[\citeproctext]{ref-geonames_about}
The GeoNames Team. (2025). \emph{GeoNames}. Geographical database and
web services. \url{https://www.geonames.org/about.html}

\bibitem[\citeproctext]{ref-wallgrun2017geocorpora}
Wallgrün, J. O., Karimzadeh, M., MacEachren, A., \& Pezanowski, S.
(2017). GeoCorpora: Building a corpus to test and train microblog
geoparsers. \emph{International Journal of Geographical Information
Science}, \emph{32}, 1--29.
