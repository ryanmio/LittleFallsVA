...
Pioneers} (C\&P) (Nugent, 1979). Without geographic coordinates,
historians and archaeologists cannot readily visualise how land
ownership evolved or test hypotheses with modern Geographic Information
System (GIS) tools. Creating a machine-readable, georeferenced version
of C\&P would unlock new quantitative approaches to long-standing
questions about colonial Virginia's social and environmental history.
...
\item
  ...
  (Bashorun, 2025), deterministic pipelines such as the GeoTxt Stanford
  NER geoparser (Karimzadeh et al., 2019), neural geoparsers like
  Mordecai-3 (Halterman, 2023), and other heuristic benchmarks in both
  error and cost?
\end{itemize}
...
of rigorous and responsible GeoAI research, as outlined by Li et al. (Li
et al., 2024) and detailed further in §2.4.
...
\item
  ...
  \emph{Cavaliers and Pioneers}, Vol. 3 (Nugent, 1979), including (i)
...
time-consuming - a genealogical case study by Julian and Abbitt (Julian
\& Abbitt, 2014) required nearly ten years of archival sleuthing and
three university-semester GIS projects to pinpoint a single family's
land claims across three Tennessee counties.
...
utility for spatial analysis (Library of Virginia, 2025). Similarly,
Loudoun County GIS staff have successfully reconstructed all original
grants within their jurisdiction (Loudoun County Office of Mapping,
2015). These initiatives demonstrate the feasibility of digitizing
historical land records but also highlight significant gaps in existing
datasets - many seventeenth- and eighteenth-century patents still lack
spatial coordinates.

Among the most thorough academic efforts for Virginia's Northern Neck
proprietary are Mitchell's (Mitchell, 1977) maps and companion text
documenting the ``Beginning at a White Oak'' patents of Fairfax County.
This work reconstructed hundreds of early land grants with polygonal
boundaries, establishing both the feasibility and research value of
transforming metes-and-bounds descriptions into spatial data. Building
on such foundations, scholars have leveraged available georeferenced
grants for substantive historical analysis. In Virginia, seminal studies
like Fausz (Fausz, 1971) utilized narrative patent abstracts to trace
settlement patterns along the James River basin, while noting the
persistent challenges of transforming textual descriptions into precise
spatial coordinates for quantitative analysis.

This analytical potential extends beyond Virginia. Dobbs (Dobbs, 2009)
used georeferenced North Carolina grants to demonstrate that
eighteenth-century town sites often followed pre-existing Indigenous
trails, while Coughlan and Nelson (Coughlan \& Nelson, 2018) leveraged a
dataset of 1,160 South Carolina grants to model settlement patterns
based on river access and soil analysis. In each case, spatial
...
DeedMapper software (Direct Line Software, 2010) helps researchers
...
names. Neural architectures such as CamCoder (Gritta et al., 2018)
...
representative example of this latest approach comes from Hu et al. (Hu
\& Kersten, 2024), who fine-tuned Llama 2-7B to generate an unambiguous
administrative string for each toponym before invoking a standard
geocoding API. Their model achieved an Accuracy@161 km of 0.90 on the
GeoCorpora set, outperforming previous neural methods and improving
toponym resolution accuracy by 13\% over the previous best neural
system. On the less ambiguous WikToR corpus the same architecture
...
(Wu et al., 2025) introduced GeoSG, a self-supervised graph neural
...
urban benchmarks. In a similar vein, Savarro et al. (Savarro et al.,
2024) demonstrated that Italian tweets can be geolocated to both
regional and point coordinates by fine-tuning decoder-only LLMs on the
GeoLingIt shared task, further confirming that pretrained language
models can internalize subtle linguistic cues of place.

Despite these advances, significant limitations remain. O'Sullivan et
al. (O'Sullivan et al., 2024) demonstrated that GPT-class models
...
challenge. Early evidence for its effectiveness comes from Hu et al. (Hu
\& Kersten, 2024), who coupled a fine-tuned Llama 2-7B with a cascading
trio of geocoders---GeoNames, Nominatim, and ArcGIS---to resolve
toponyms the model had already disambiguated linguistically. Their
experiments demonstrated that this hybrid pipeline raised Accuracy@161
km by 7--17 percentage points relative to either component used in
isolation.

Extending this concept to more complex natural language descriptions,
Huang et al. (Huang et al., 2024) developed GeoAgent for free-form
...
pattern known as the ReAct prompting paradigm (Yao et al., 2023), which
...
initiative (Schottlander \& Shekel, 2025) exemplifies the integration of
foundation models with Earth Engine, BigQuery, and Maps Platform. This
system enables agentic LLMs to chain satellite imagery, socioeconomic
layers, and routing services to answer compound spatial queries in
seconds---a capability relevant to both consumer applications and
research contexts.
...
(Hu et al. (Hu \& Kersten, 2024); Huang et al. (Huang et al., 2024)).
The present work builds on this pattern by testing whether a similar
benefit materializes for colonial land-grant geolocation---comparing a
pure one-shot prompt to a tool-augmented chain-of-thought that can issue
...
responsibility}, which Li et al. (Li et al., 2024) identify as four
...
\emph{Cavaliers \& Pioneers}, Volume 3 (Nugent, 1979) contains 5,471
abstracts of Virginia land patents recorded in patent books 9--14
(1695--1732). The digitized corpus (Mioduski, 2025) provides
...
History (One Shared Story, 2020). Matching relied on grantee name, grant
year, and acreage. Each candidate polygon was visually audited against
modern hydrography, historic county boundaries, and the neighbouring
patent topology; only polygons whose centroid plausibly sat on the
rivers, creeks, or adjoining grants described in the abstract were
retained. The centroid of each verified polygon serves as the reference
coordinate for that land grant.
...
A certified GIS analyst (Bashorun, 2025) implemented an automated
geolocating procedure leveraging standard geospatial libraries and
toolsets. The analyst was selected through a competitive bidding process
on a freelancer marketplace, where 49 qualified contractors submitted
bids averaging \$133 USD (range: \$30-\$250). The selected contractor
holds an MSc in Geography \& Environmental Management with 9+ years of
experience in geospatial analysis and maintains a 5.0-star rating with
100\% on-time delivery record. The workflow ingested the patent texts,
tokenized toponyms, and queried a multi-layered gazetteer stack
...
billable hours for all 43 grants with verified ground truth. This
end-to-end workflow time represents the total cost of bespoke GIS
analysis, contrasting with off-the-shelf LLM inference that requires no
custom development.
...
(Halterman, 2023), augmented with domain-specific heuristics tuned for
colonial Virginia deeds (full details in Appendix B.1). In brief, the
pipeline
...
\subsection{4.4 County-Centroid Baseline
...
insights into tool-augmented performance.
...
If at least three predictions fall within the same 0.5 km cluster, their
spherical centroid becomes the final answer; otherwise the centroid of
all five points is returned. This majority-vote strategy reduces random
scatter and mitigates occasional large-error outliers. The ensemble
(method E-1) achieves a mean error of 18.7 km---the best of all
evaluated methods---at roughly 5× the token cost of a single o3 call but
still two orders of magnitude cheaper than the GIS benchmark. A
...
43). \{\#tbl:reasoning\}}\tabularnewline
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
ID
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.1579}}
  >{\raggedright\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.1579}}@{}}
\caption{LLM tool-chain behavior on the 43-grant test set.
...
appeared in fewer than one call per ten. gpt-4.1-2025-04-14 (T-4)
adopted a more economical strategy, issuing on average 2.3 calls per
grant while succeeding on the first query in 73\% of cases. The
...
\item
  ...
  Method rankings remain unchanged; only H-1 (Bashorun, 2025) (−6.6 km)
  and M-6 (−6.3 km) show material shifts.
\item
  ...
  only slightly---from 18.7 km to 20.4 km---and the ≤10 km hit-rate fell
  by just 4.6 pp (39.5 → 34.9). The mild degradation indicates that the
...
\item
  ...
  Shared Story (One Shared Story, 2020) and other spatial datasets used
  for verification could potentially appear in training corpora,
...
\item
  ...
  reproducibility.
\item
  \textbf{GIS benchmark generality.} The GIS analyst baseline (Bashorun,
  2025) relies on a single expert-authored geocoding procedure. Accuracy
  and efficiency might vary significantly with different gazetteer
  sources, methods, parameter tuning, or analyst expertise. Therefore,
  this single-practitioner workflow is best interpreted as a practical
  lower bound or illustrative benchmark, rather than a representative or
...
\item
  ...
  \emph{Cavaliers and Pioneers} (Nugent, 1979)---and analogous land
  books from Maryland and North Carolina---would permit a cross-colonial
  analysis of settlement diffusion.
\item
...
are deeply grateful to Bimbola Bashorun (Bashorun, 2025) for providing
the professional GIS benchmark that was crucial to evaluating model
performance.
...
\NormalTok{• \textasciigrave{}geocode\_place(query, strategy)\textasciigrave{}}
\NormalTok{    – Look up a place name via the Google Geocoding API (Virginia{-}restricted).}
\NormalTok{    – Returns JSON: \textasciigrave{}\{lat, lng, formatted\_address, strategy, query\_used\}\textasciigrave{}.}
\NormalTok{• \textasciigrave{}compute\_centroid(points)\textasciigrave{}}
\NormalTok{    – Accepts **two or more** objects like \textasciigrave{}\{lat: 37.1, lng: {-}76.7\}\textasciigrave{} and returns their average.}
...
\NormalTok{1. Call \textasciigrave{}geocode\_place\textasciigrave{} with that string. If the result is in the expected or an adjacent county *and* the feature lies in Virginia (or an NC border county), treat it as **plausible**. A matching feature keyword in \textasciigrave{}formatted\_address\textasciigrave{} is *preferred* but not mandatory after several attempts.}

\NormalTok{2. If the first call is not plausible, iteratively refine the query (alternate spelling, nearby landmark, bordering county, etc.) and call \textasciigrave{}geocode\_place\textasciigrave{} again until you obtain *at least one* plausible point **or** you have made six tool calls, whichever comes first.}

\NormalTok{3. Optional centroid use – if the grant text clearly places the tract *between* two or more natural features (e.g., "between the mouth of Cypress Swamp and Blackwater River") **or** you have two distinct plausible anchor points (creek{-}mouth, swamp, plantation), you may call \textasciigrave{}compute\_centroid(points)\textasciigrave{} exactly once to average them. Otherwise skip this step.}
...
\NormalTok{5. Final answer – reply with **only** the coordinates in decimal degrees with six digits after the decimal point, e.g., \textasciigrave{}36.757059, {-}77.836728\textasciigrave{}. No explanatory text.}

\NormalTok{Important rules}
\NormalTok{• Always perform at least one successful \textasciigrave{}geocode\_place\textasciigrave{} call before any other tool.}
\NormalTok{• Invoke \textasciigrave{}compute\_centroid\textasciigrave{} only when you already have two or more plausible anchor points and averaging will help locate a "between" description.}
\NormalTok{• Never invent coordinates—derive them from tool output.}
\NormalTok{• Return no explanatory text, symbols, or degree signs—just \textasciigrave{}lat, lon\textasciigrave{}.}
\end{Highlighting}
\end{Shaded}
...
\caption{Mean error with 95\% confidence intervals for each method.
\{\#tbl:error\_ci\}}\tabularnewline
\toprule\noalign{}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 22\tabcolsep) * \real{0.0833}}
  >{\raggedright\arraybackslash}p{(\linewidth - 22\tabcolsep) * \real{0.0833}}@{}}
\caption{Detailed performance metrics by method.
\{\#tbl:method\_performance\}}\tabularnewline
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
Method
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.1500}}
  >{\raggedright\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.1500}}@{}}
\caption{Cost-accuracy trade-off by model family.
\{\#tbl:cost\_accuracy\}}\tabularnewline
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
Model
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2500}}
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2500}}@{}}
\caption{Processing time by model. \{\#tbl:time\_usage\}}\tabularnewline
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
Model
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
\begin{longtable}[]{@{}llll@{}}
\caption{Token consumption by model across all test runs.
\{\#tbl:token\_usage\}}\tabularnewline
\toprule\noalign{}
...
professional GIS benchmark (H-1) (Bashorun, 2025) results, categorized
by precision level. This breakdown reveals that even with expert domain
knowledge and access to specialized historical gazetteers, more than
41.9\% of the human-geocoded grants were located at only state-level
precision. ``High'' indicates grants where both county boundaries and
specific landmarks were used; ``Medium'' indicates county-centroid
placement; ``Low'' indicates state-level precision only.
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.1579}}
  >{\raggedright\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.1579}}@{}}
\caption{Professional GIS benchmark results by accuracy category.
\{\#tbl:human\_benchmark\}}\tabularnewline
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
Accuracy Category
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{../analysis/figures/error_boxplot.pdf}}
...
Grant 1 (left) shows a success case where the o3 model (M-2) and
tool-chain gpt-4.1 (T-4) are close to ground truth. Grant 19 (right)
illustrates a failure mode where an early spurious geocoder hit sends
the tool-chain prediction far from ground truth, whereas the unguided
model remains closer to the actual location. Basemap © OSM.

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{../analysis/mapping_workflow/contact_sheet.png}}
\caption{Grid view of all 43 mapped grants showing ground truth and
predictions.}\label{fig:all-grants}
\end{figure}

Figure \ref{fig:all-grants} plots all six methods for every locatable
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2500}}
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2500}}@{}}
\caption{Marginal cost to improve the ≤10 km hit-rate by one percentage
point. \{\#tbl:cost\_marginal\}}\tabularnewline
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
Model
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{../analysis/figures/pareto_latency_tradeoff.pdf}}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 22\tabcolsep) * \real{0.0732}}
  >{\raggedright\arraybackslash}p{(\linewidth - 22\tabcolsep) * \real{0.0732}}@{}}
\caption{Comparison of identical models with and without tool access.
\{\#tbl:tool\_direct\_comparison\}}\tabularnewline
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
Model
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 16\tabcolsep) * \real{0.1071}}
  >{\raggedright\arraybackslash}p{(\linewidth - 16\tabcolsep) * \real{0.1071}}@{}}
\caption{Quantified effect of tool augmentation.
\{\#tbl:tool\_effect\}}\tabularnewline
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
Model
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 20\tabcolsep) * \real{0.0909}}
  >{\raggedright\arraybackslash}p{(\linewidth - 20\tabcolsep) * \real{0.0909}}@{}}
\caption{Best-performing methods per category.
\{\#tbl:best\_methods\}}\tabularnewline
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
Method
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
\caption{Distribution of tool calls by method and tool type.
\{\#tbl:tool\_distribution\}}\tabularnewline
\toprule\noalign{}
...
\begin{longtable}[]{@{}llll@{}}
\caption{Tool search efficiency metrics.
\{\#tbl:search\_efficiency\}}\tabularnewline
\toprule\noalign{}
...
\bibitem[\citeproctext]{ref-Bashorun2025_gis}
Bashorun, B. (2025). \emph{Colonial virginia land grants geocoding
dataset}. Professional GIS Services.

\bibitem[\citeproctext]{ref-Coughlan2018_settlement}
Coughlan, M. R., \& Nelson, D. R. (2018). Influences of native american
land use on the colonial euro-american settlement of the south carolina
piedmont. \emph{PLOS ONE}, \emph{13}(3), e0195036.
\url{https://doi.org/10.1371/journal.pone.0195036}

\bibitem[\citeproctext]{ref-DeedMapper_software}
Direct Line Software. (2010). \emph{DeedMapper 4.2}. Commercial software
for historical deed mapping. \url{https://directlinesoftware.com/deeds/}

\bibitem[\citeproctext]{ref-Dobbs2009_backcountry}
Dobbs, G. R. (2009). Backcountry settlement development and indian
trails: A GIS land-grant analysis. \emph{Social Science Computer
Review}, \emph{27}(3), 331--347.
\url{https://doi.org/10.1177/0894439308329760}

\bibitem[\citeproctext]{ref-Fausz1971_settlement}
Fausz, J. F. (1971). \emph{Patterns of settlement in the {James} {River}
{Basin}, 1607-1642} {[}Master\textquotesingle s thesis, College of
William; Mary{]}. \url{https://dx.doi.org/doi:10.21220/s2-jedw-z031}

\bibitem[\citeproctext]{ref-Gritta2018_camcoder}
Gritta, M., Pilehvar, M. T., \& Collier, N. (2018). Which melbourne?
Augmenting geocoding with maps. \emph{Proceedings of the 56th Annual
Meeting of the Association for Computational Linguistics (ACL 2018)},
1285--1296. \url{https://doi.org/10.17863/CAM.27796}

\bibitem[\citeproctext]{ref-halterman2023mordecai}
Halterman, A. (2023). \emph{Mordecai 3: A neural geoparser and event
geocoder}. \url{https://doi.org/10.48550/arXiv.2303.13675}

\bibitem[\citeproctext]{ref-Hu2024_toponym_llm}
Hu, X., \& Kersten, J. (2024). Enhancing toponym resolution with
fine-tuned {LLMs} ({Llama2}). \emph{Proceedings of the Second
International Workshop on Geographic Information Extraction from Texts
at ECIR 2024 (GeoExT@ECIR)}, 52--56.

\bibitem[\citeproctext]{ref-Huang2024_geoagent}
Huang, C., Chen, S., Li, Z., Qu, J., Xiao, Y., Liu, J., \& Chen, Z.
(2024). GeoAgent: To empower LLMs using geospatial tools for address
standardization. \emph{Findings of the Association for Computational
Linguistics: ACL 2024}, 6048--6063.
\url{https://doi.org/10.18653/v1/2024.findings-acl.362}

\bibitem[\citeproctext]{ref-Julian2014_tennessee}
Julian, G. M., \& Abbitt, R. J. F. (2014). GIS and genealogy: Teaching
GIS while learning about the past. \emph{ArcUser}.
\url{https://www.esri.com/about/newsroom/arcuser/gis-and-genealogy}

\bibitem[\citeproctext]{ref-karimzadeh2019geotxt}
Karimzadeh, M., Pezanowski, S., MacEachren, A., \& Wallgrün, J. O.
(2019). GeoTxt: A scalable geoparsing system for unstructured text
geolocation. \emph{Transactions in GIS}, \emph{23}, 118--136.
\url{https://doi.org/10.1111/tgis.12510}

\bibitem[\citeproctext]{ref-Li2024_geoai}
Li, W., Janowicz, K., Mai, G., Goodchild, M. F., Gautier, J., Kuhn, W.,
\& al., et. (2024). GeoAI for science and the science of GeoAI.
\emph{Journal of Spatial Information Science}, \emph{29}, 1--17.
\url{https://doi.org/10.5311/JOSIS.2024.29.349}

\bibitem[\citeproctext]{ref-lva_patents_db}
Library of Virginia. (2025). \emph{Virginia land office patents and
grants online database}. Web portal.
\url{https://lva-virginia.libguides.com/land-grants}

\bibitem[\citeproctext]{ref-loudoun_grants_dataset}
Loudoun County Office of Mapping. (2015). \emph{Original land grants of
loudoun county, VA {[}GIS dataset{]}}. Interactive ArcGIS WebMap.
\url{https://www.arcgis.com/apps/MapTools/index.html?appid=d3c4ba1031564f919ca28c9bb5a48350}

\bibitem[\citeproctext]{ref-mioduski_2025_cvp3}
Mioduski, R. (2025). \emph{Colonial virginia land-grant OCR corpus (vol
3)}. Zenodo. Version 1.0.0.
\url{https://doi.org/10.5281/zenodo.16269949}.
\url{https://doi.org/10.5281/zenodo.16269949}

\bibitem[\citeproctext]{ref-mitchell1977whiteoak}
Mitchell, B. (1977). \emph{Beginning at a white oak: Patents and
northern neck grants of fairfax county, virginia}. Office of
Comprehensive Planning, Fairfax County.
\url{https://www.fairfaxcounty.gov/history-commission/sites/history-commission/files/assets/documents/resources/map.pdf}

\bibitem[\citeproctext]{ref-Nugent1979_cavaliers3}
Nugent, N. M. (1979). \emph{Cavaliers and pioneers: Abstracts of
virginia land patents and grants, volume three: 1695-1732}. Virginia
State Library.

\bibitem[\citeproctext]{ref-Osullivan2024_metric}
O'Sullivan, K., Schneider, N. R., \& Samet, H. (2024). Metric reasoning
in large language models. \emph{Proceedings of the 32nd ACM SIGSPATIAL
International Conference on Advances in Geographic Information Systems
(SIGSPATIAL '24)}, 501--504.
\url{https://doi.org/10.1145/3678717.3691226}

\bibitem[\citeproctext]{ref-central_va_patents_gis}
One Shared Story. (2020). \emph{Early virginia land patent - estimated
polygons {[}CentralVAPatents\_PLY{]}}. ArcGIS Online Feature Service.
\url{https://www.arcgis.com/home/item.html?id=6fc329e0694b497196435beba8244a47}

\bibitem[\citeproctext]{ref-savarro2024geolingit}
Savarro, D., Zago, D., \& Zoia, S. (2024). Leveraging large language
models to geolocate linguistic variations in social media posts.
\emph{arXiv Preprint arXiv:2407.16047}.
\url{https://doi.org/10.48550/arXiv.2407.16047}

\bibitem[\citeproctext]{ref-GoogleResearch2025_geospatial}
Schottlander, D., \& Shekel, T. (2025). \emph{Geospatial reasoning:
...
\bibitem[\citeproctext]{ref-wu2025geosg}
Wu, Y., Zeng, Z., Liu, K., Xu, Z., Ye, Y., Zhou, S., Yao, H., \& Li, S.
(2025). Text geolocation prediction via self-supervised learning.
\emph{ISPRS International Journal of Geo‐Information}, \emph{14}(4),
170. \url{https://doi.org/10.3390/ijgi14040170}
...
\emph{Proceedings of the 11th International Conference on Learning
Representations}. \url{https://doi.org/10.48550/arXiv.2210.03629}
