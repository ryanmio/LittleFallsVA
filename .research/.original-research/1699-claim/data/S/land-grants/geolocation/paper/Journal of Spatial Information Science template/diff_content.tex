...
Figure \ref{fig:cdf_models} plots \DIFdelbegin \DIFdel{cumulative }\DIFdelend \DIFaddbegin \DIFadd{the cumulative distribution function
(CDF) of }\DIFaddend accuracy as a function of distance threshold\DIFdelbegin \DIFdel{; curves that are higher and further left indicate
stronger performance across thresholds. }\DIFdelend \DIFaddbegin \DIFadd{. The CDFs
...
\DIFaddend \caption{\label{tbl:cost}Cost metrics by method---cost per \DIFdelbegin \DIFdel{located grant
(USD) and cost per }\DIFdelend 1,000 located
grants (USD) \DIFdelbegin \DIFdel{, with }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend mean error (km).}\tabularnewline
\toprule\noalign{}
...
Full reasoning chains are available in \DIFdelbegin \DIFdel{appendix}\DIFdelend \DIFaddbegin \DIFadd{Appendix A.3}\DIFaddend .
...
\section{\DIFdelbegin \DIFdel{12 }\DIFdelend Use of Artificial
...
\section{\DIFdelbegin \DIFdel{13 }\DIFdelend Acknowledgements}\label{acknowledgements}
...
\DIFaddbegin \section*{\DIFadd{Appendices}}\label{appendices}

\appendix\DIFaddend \section{Supplementary Methods \&
...
}\href{https://github.com/ryanmio/colonial-virginia-llm-geolocation/blob/main/paper/APPENDIX_B_Extended_Results.md}{\DIFadd{github.com/ryanmio/colonial-virginia-llm-geolocation}}\DIFaddend .
...
\DIFdel{Mean error with 95\% bootstrap CIs by
method; reference for
Figure\textasciitilde{}\ref{fig:accuracy_bar}.}}%DIFAUXCMD
...
\DIFdel{Full performance profile per
method (dispersion and ≤10/25/50 km bands).}}%DIFAUXCMD
%DIFDELCMD < \tabularnewline
%DIFDELCMD < \toprule\noalign{}
...
\DIFdel{Processing time by model; companion to
Section\textasciitilde6.3 for throughput estimates.}}%DIFAUXCMD
%DIFDELCMD < \tabularnewline
%DIFDELCMD < \toprule\noalign{}
%DIFDELCMD < \begin{minipage}[b]{\linewidth}\raggedright
%DIFDELCMD < %%%
...
\DIFdel{Token consumption by model; informs
compute budgeting and reproducibility.}}%DIFAUXCMD
%DIFDELCMD < \tabularnewline
%DIFDELCMD < \toprule\noalign{}
%DIFDELCMD < %%%
\DIFdel{Model }%DIFDELCMD < & %%%
...
\DIFdel{Professional GIS benchmark by
placement precision (overall; High/Medium/Low).}}%DIFAUXCMD
%DIFDELCMD < \tabularnewline
%DIFDELCMD < \toprule\noalign{}
%DIFDELCMD < \begin{minipage}[b]{\linewidth}\raggedright
%DIFDELCMD < %%%
...
}\href{https://github.com/ryanmio/colonial-virginia-llm-geolocation/blob/main/paper/APPENDIX_C_Supplementary_Figures.md}{\DIFadd{github.com/ryanmio/colonial-virginia-llm-geolocation}}\DIFadd{.
}\DIFaddend 
...
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Error boxplots by method; complementary to violin and CDF
plots.}}%DIFAUXCMD
...
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{All mapped grants with ground truth (black stars) and
predictions; dashed lines indicate errors.}}%DIFAUXCMD
...
\DIFdel{Marginal cost per +1 pp gain in ≤10 km
accuracy.}}%DIFAUXCMD
%DIFDELCMD < \tabularnewline
%DIFDELCMD < \toprule\noalign{}
%DIFDELCMD < \begin{minipage}[b]{\linewidth}\raggedright
...
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Latency--accuracy scatter (seconds, log x‑axis vs mean
error).}}%DIFAUXCMD
