...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{This appendix has been moved to the supplementary repository for space
and readability. See the public supplement (tag:
R1-supplement-2025-10-23):
}\DIFaddend 
...
increase (5,937,907 → 6,742,767).}%DIFDELCMD < 
...
error for each method as a boxplot, complementing the violin plot and CDF already presented in the main text. The boxplot highlights the median error (central line), interquartile range (box), and outliers
...
%DIFDELCMD < %%%
\DIFdel{Figure \ref{fig:contactsheet} plots all six methods for every locatable
...
mean-error vs.~cost Pareto plot in the main text:
it quantifies the
...
%DIFDELCMD < %%%
\DIFdel{Cost per 1 k located (USD)}%DIFDELCMD < \end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
%DIFDELCMD < %%%
\DIFdel{Cost per +1 \% ≤10 km hit (USD)
}%DIFDELCMD < \end{minipage} \\
%DIFDELCMD < \midrule\noalign{}
