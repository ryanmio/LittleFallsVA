...
al.\DIFaddbegin \DIFadd{\textasciitilde{}}\DIFaddend \citep{Li2024_geoai} and detailed further in §2.4.
...
on the GeoCorpora set \DIFaddbegin \DIFadd{(a benchmark of geotagged microtexts for toponym
resolution) \mbox{%DIFAUXCMD
\citep{karimzadeh2019geocorpora}}\hskip0pt%DIFAUXCMD
}\DIFaddend , outperforming previous
neural methods and improving toponym resolution accuracy by 13\% over
the previous best neural system. On the less ambiguous WikToR corpus \DIFaddbegin \DIFadd{(a
Wikipedia-based corpus for region classification and toponym resolution)
\mbox{%DIFAUXCMD
\citep{gritta2018wiktor} }\hskip0pt%DIFAUXCMD
}\DIFaddend the same architecture reached 0.98. Crucially,
...
cascading trio of geocoders---GeoNames \DIFdelbegin \DIFdel{, Nominatim, and ArcGIS---to
}\DIFdelend \DIFaddbegin \DIFadd{gazetteer \mbox{%DIFAUXCMD
\citep{geonames}}\hskip0pt%DIFAUXCMD
,
Nominatim (OpenStreetMap geocoder) \mbox{%DIFAUXCMD
\citep{nominatim}}\hskip0pt%DIFAUXCMD
, and ArcGIS World
Geocoding Service \mbox{%DIFAUXCMD
\citep{arcgis_world_geocoding}}\hskip0pt%DIFAUXCMD
---to }\DIFaddend resolve toponyms
...
\citep{geoglue} }\hskip0pt%DIFAUXCMD
}\DIFaddend and an in-house Chinese address dataset, demonstrating
...
responsibility}, which Li et al.\DIFaddbegin \DIFadd{\textasciitilde{}}\DIFaddend \citep{Li2024_geoai}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.1304}}
  >{\raggedright\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.1304}}@{}}
\caption{\label{tbl:accuracy}\DIFdelbegin \DIFdel{Coordinate-accuracy
...
Figure \ref{fig:cdf_models} \DIFdelbegin \DIFdel{presents the cumulative distribution of
errors for each evaluated method}\DIFdelend \DIFaddbegin \DIFadd{plots cumulative accuracy as a function of
distance threshold; curves that are higher and further left indicate
stronger performance across thresholds}\DIFaddend .
...
\caption{\label{tbl:cost}Cost \DIFdelbegin \DIFdel{efficiency of evaluated
methods}\DIFdelend \DIFaddbegin \DIFadd{metrics by method---cost per located grant
(USD) and cost per 1,000 located grants (USD), with mean error
(km)}\DIFaddend .}\tabularnewline
\toprule\noalign{}
...
\caption{\label{tbl:error_ci}Mean error with 95\% \DIFdelbegin \DIFdel{confidence intervals
for each }\DIFdelend \DIFaddbegin \DIFadd{bootstrap CIs by
}\DIFaddend method\DIFaddbegin \DIFadd{; reference for
Figure\textasciitilde{}\ref{fig:accuracy_bar}}\DIFaddend .}\tabularnewline
\toprule\noalign{}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 22\tabcolsep) * \real{0.0833}}
  >{\raggedright\arraybackslash}p{(\linewidth - 22\tabcolsep) * \real{0.0833}}@{}}
\caption{\label{tbl:method_performance}\DIFdelbegin \DIFdel{Detailed }\DIFdelend \DIFaddbegin \DIFadd{Full }\DIFaddend performance \DIFdelbegin \DIFdel{metrics by
}\DIFdelend \DIFaddbegin \DIFadd{profile per
}\DIFaddend method \DIFaddbegin \DIFadd{(dispersion and ≤10/25/50 km bands)}\DIFaddend .}\tabularnewline
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
Method
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.1500}}
  >{\raggedright\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.1500}}@{}}
\caption{\label{tbl:cost_accuracy}\DIFdelbegin \DIFdel{Cost-accuracy trade-off }\DIFdelend \DIFaddbegin \DIFadd{Cost--accuracy summary }\DIFaddend by model
family\DIFaddbegin \DIFadd{; use with Figure\textasciitilde{}\ref{fig:pareto_cost} to select
an operating point}\DIFaddend .}\tabularnewline
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
Model
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2500}}
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2500}}@{}}
\caption{\label{tbl:time_usage}Processing time by model\DIFaddbegin \DIFadd{; companion to
Section\textasciitilde6.3 for throughput estimates}\DIFaddend .}\tabularnewline
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
Model
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
\begin{longtable}[]{@{}llll@{}}
\caption{\label{tbl:token_usage}Token consumption by model\DIFdelbegin \DIFdel{across all
test runs}\DIFdelend \DIFaddbegin \DIFadd{; informs
compute budgeting and reproducibility}\DIFaddend .}\tabularnewline
\toprule\noalign{}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.1579}}
  >{\raggedright\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.1579}}@{}}
\caption{\label{tbl:human_benchmark}Professional GIS benchmark \DIFdelbegin \DIFdel{results
}\DIFdelend by
\DIFdelbegin \DIFdel{accuracy category}\DIFdelend \DIFaddbegin \DIFadd{placement precision (overall; High/Medium/Low)}\DIFaddend .}\tabularnewline
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
Accuracy Category
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
\caption{Error \DIFdelbeginFL \DIFdelFL{Distribution Boxplot }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{boxplots }\DIFaddendFL by \DIFdelbeginFL \DIFdelFL{Method}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{method; complementary to violin and CDF
plots.}\DIFaddendFL }\label{fig:error_boxplot}
\end{figure}
...
\caption{\DIFdelbeginFL \DIFdelFL{Grid view of all 43 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{All }\DIFaddendFL mapped grants \DIFdelbeginFL \DIFdelFL{showing }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{with }\DIFaddendFL ground truth \DIFaddbeginFL \DIFaddFL{(black stars) }\DIFaddendFL and
predictions\DIFaddbeginFL \DIFaddFL{; dashed lines indicate errors}\DIFaddendFL .}\label{fig:contactsheet}
\end{figure}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2500}}
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2500}}@{}}
\caption{\label{tbl:cost_marginal}Marginal cost \DIFdelbegin \DIFdel{to improve the }\DIFdelend \DIFaddbegin \DIFadd{per +1 pp gain in }\DIFaddend ≤10 km
\DIFdelbegin \DIFdel{hit-rate by one percentage point}\DIFdelend \DIFaddbegin \DIFadd{accuracy}\DIFaddend .}\tabularnewline
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
Model
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
\caption{\DIFdelbeginFL \DIFdelFL{Latency-Accuracy Tradeoff}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Latency--accuracy scatter (seconds, log x‑axis vs mean
error).}\DIFaddendFL }\label{fig:pareto_latency}
\end{figure}
