...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{Appendix B (Extended Results) is available in the supplementary
repository (tag: R1-supplement-2025-10-23):
}\DIFaddend 

\DIFdelbegin \DIFdel{Table \ref{tbl:error_ci} provides comprehensive error statistics for
...
\DIFdel{km.
}\DIFdelend \DIFaddbegin \DIFadd{https://github.com/ryanmio/colonial-virginia-llm-geolocation/tree/R1-supplement-2025-10-23/paper/APPENDIX\_B\_Extended\_Results.md
}\DIFaddend 
...
kilometers of ground truth.}%DIFDELCMD < 
...
%DIFDELCMD < %%%
\DIFdel{Table \ref{tbl:cost_accuracy} examines the cost-accuracy relationship,
...
%DIFDELCMD < %%%
\DIFdel{Cost per 1k located (USD) }%DIFDELCMD < \end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
%DIFDELCMD < %%%
\DIFdel{Cost per +1\% ≤10 km hit (USD) }%DIFDELCMD < \end{minipage} \\
%DIFDELCMD < \midrule\noalign{}
%DIFDELCMD < \endfirsthead
%DIFDELCMD < \toprule\noalign{}
%DIFDELCMD < \begin{minipage}[b]{\linewidth}\raggedright
...
%DIFDELCMD < %%%
\DIFdel{Cost per1k located (USD) }%DIFDELCMD < \end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
%DIFDELCMD < %%%
...
architecture: adding tools to gpt-4.1-2025-04-14 increased token usage by 18.3× (176,698 → 3,229,140) , while o4-mini showed only a 1.14×
increase (5,937,907 → 6,742,767).
}%DIFDELCMD < 

%DIFDELCMD < %%%
\subsection{\DIFdel{Professional GIS Benchmark
Analysis}}%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
...
}\DIFdelend \DIFaddbegin \DIFadd{Contents: (i) detailed accuracy metrics with bootstrap CIs; (ii) full
per‑method performance tables; (iii) cost--accuracy summaries; (iv)
processing time analysis; (v) token usage statistics; (vi) }\DIFaddend professional
GIS benchmark \DIFdelbegin \DIFdel{(H-1) \mbox{%DIFAUXCMD
\citep{Bashorun2025_gis} }\hskip0pt%DIFAUXCMD
results,
categorized by precision level. This breakdownreveals that even with
...
``High'' indicates grants where both county
...
methods except gpt-3.5-turbo (M-6)}\DIFdelend \DIFaddbegin \DIFadd{breakdown}\DIFaddend .
\DIFdelbegin \DIFdel{This underscores the inherent
difficulty of the task and further highlights the significance of the
accuracy improvements achieved by the LLM approaches.}\DIFdelend 
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{Appendix C (Supplementary Figures) is available in the supplementary
repository (tag: R1-supplement-2025-10-23):
}\DIFaddend 
...
(points) for each method, providing a clear view of error distribution and central tendency.
}\DIFdelend \DIFaddbegin \DIFadd{https://github.com/ryanmio/colonial-virginia-llm-geolocation/tree/R1-supplement-2025-10-23/paper/APPENDIX\_C\_Supplementary\_Figures.md
}\DIFaddend 
...
%DIFDELCMD < %%%
\DIFdel{Figure \ref{fig:contactsheet} plotsall six methods for every locatable
...
cartographic clarity the H-2, H-3, and H-4 baselines are omitted;
their
...
requires to raise the }\DIFdelend \DIFaddbegin \DIFadd{Contents: (i) error distribution plots; (ii) contact‑sheet error maps;
(iii) marginal cost of }\DIFaddend ≤10 km \DIFdelbegin \DIFdel{hit-rate by one percentage point (column
...
\DIFdel{geocoding rather than average error
alone.
}%DIFDELCMD < 
...
``high-precision'' accuracycosts only six cents---roughly two orders of
...
of ≈502 s per grant. Note the logarithmic scale on the x-axis}\DIFdelend \DIFaddbegin \DIFadd{accuracy; (iv) latency--accuracy tradeoff}\DIFaddend .
