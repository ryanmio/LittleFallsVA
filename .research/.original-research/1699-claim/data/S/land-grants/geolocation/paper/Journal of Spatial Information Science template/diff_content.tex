...
mental triangulation against its latent world map, and produces a best-guess point estimate}\DIFdelend \DIFaddbegin \DIFadd{maps the input text directly to a
coordinate using the model's learned representations, with no external
tools. All computation occurs within the model's parameters: the system
produces a single coordinate based on learned statistical associations
and implicit spatial priors; no tool calls are issued}\DIFaddend . By contrast, the
