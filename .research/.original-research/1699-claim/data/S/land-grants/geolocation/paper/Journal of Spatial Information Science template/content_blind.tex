...
\item
  ...
  excerpts of up to 200 words for the 45 evaluation abstracts. The full
...
\item
  ...
  and other archival sources, providing a high-fidelity evaluation
  target.\\
\item
...
Additionally, the full OCR-corrected corpus of 5,471 abstracts, 45
...
each) of the 45 abstracts with authoritative ground-truth points are
...
established through a rigorous two-step verification process:
...
\item
  ...
  grantee name, year, and acreage), the centroid of the GIS polygon was
  used as the authoritative coordinate.
\item
  Secondary method: For grants without a matching GIS polygon, published
  historical maps and archival sources were consulted. If the grant
  could be confidently located on these maps and aligned with modern
  coordinates, a ground-truth point was assigned and the source was
  cited in the bibliography.
\end{enumerate}

The 36\% locatability rate reflects an important methodological choice.
The study deliberately avoided expanding the ground-truth set beyond
...
hours for 50 grants. This study treats this end-to-end
...
\item
  ...
  km across the grid). Given the marginal gains, \textbf{t = 0.8} is
  fixed for GPT-4.1 and the default \textbf{t = 0.0} is kept for GPT-4o
  in all downstream benchmarks.
\end{itemize}
...
(GPT-4o), such assisted verification remains an order of magnitude
cheaper than start-to-finish traditional geocoding.
...
family.\footnote{Three ultra-low-cost GPT-4-class variants
  (GPT-4.1-mini, GPT-4.1-nano, GPT-4o-mini) were also evaluated. Their
  outputs rarely conformed to the required coordinate format, yielding a
  mean error of ≈ 49 km; details are archived in the supplementary
  repository
...
error by \textless1 km. Likewise, GPT-4.1 accuracy varies by only ±1.5
km across the 0--1.2 temperature range, and GPT-4o shows no systematic
trend. In practice, therefore, the default (cheap) settings already sit
near the cost-accuracy frontier.
...
\item
  ...
  later patent books, or on neighboring colonies with different
...
combinations, the findings show that an off-the-shelf one-shot prompt to
the o3-2025-04-16 model achieves a mean positional error of 23.4
...
Virginia's colonial land records for generations of scholars. The
authors are deeply grateful to Bimbola Bashorun \citep{Bashorun2025_gis}
for providing the professional GIS benchmark that was crucial to
evaluating model performance. Special thanks to the Library of Virginia
and the Virginia Surveyor's Office for granting access to their digital
archives and land patent collections, which made the ground-truth
dataset possible.
