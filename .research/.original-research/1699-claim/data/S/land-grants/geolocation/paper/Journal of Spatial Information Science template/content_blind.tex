...
independent one-shot calls to \texttt{o3-2025-04-16},
...
\item
  ...
  \texttt{[NAME]}. If the model had memorised
  ...
  error rose only slightly---from 19.2 km to 20.6 km---and the ≤10 km
  hit-rate fell by just 4.7 pp (34.9 → 30.2). The mild degradation
...
OpenAI \emph{Responses} request via the \texttt{tools}
parameter, allowing the model to emit
\texttt{function\_call} objects whose arguments are
validated and then executed by the evaluation driver. After execution
the Python backend streams a
\texttt{function\_call\_output} item containing the
...
The driver maps the call to \texttt{google\_geocode()}
with a hard-coded
\texttt{components=administrative\_area:VA} filter,
discards results falling outside Virginia, and returns a trimmed JSON
object
\texttt{\{lat, lng, formatted\_address, strategy, query\_used\}}.
...
The backend converts each \texttt{(lat, lng)} pair to a
...
\item
  \emph{Invocation.} It emits a \texttt{function\_call}
...
\item
  ...
  \texttt{function\_call} requests to the tool back-end
  described above,
\item
  logs raw API traffic---including intermediate tool traces---to
  \texttt{runs/<method>/calls.jsonl}, and
\item
  emits both row-level results
  (\texttt{results\_<evalset>.csv}) and a Markdown run
...
\href{[Repository URL removed for blind review]/blob/main/paper/APPENDIX_B_Extended_Results.md}{github.com/ryanmio/colonial-virginia-llm-geolocation}.
...
\href{[Repository URL removed for blind review]/blob/main/paper/APPENDIX_C_Supplementary_Figures.md}{github.com/ryanmio/colonial-virginia-llm-geolocation}.
...
\section{Leakage Audit}\label{appendix-e-leakage-audit}
